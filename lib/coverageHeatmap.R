...
# average plot ------------------------------------------------------------
averageCovPlot <- function(data_mat, plot_paras, sample_name){
...
plotMain <- function(plot_paras){
  ...
  for (i in 1:length(heatmap_files)) {
    ...
    ptop <- averageCovPlot(data_mat, plot_paras, sample_name)
...
mycol <- brewer.pal(n = 3, name = "Set1")
...
mycol <- brewer.pal(n = 3, name = "Set1")
mycol <- mycol[1]
# 查看提取到的颜色（通常是十六进制颜色代码）
print(mycol)

library(ggsci)
mycol <- brewer.pal(n = 3, name = "aaas")
