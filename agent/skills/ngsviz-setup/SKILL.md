...
1. Receive the absolute ngsViz installation root resolved by `$ngsviz-agent`. Identify the directory containing `README.md`, `agent/`, `lib/`, and the ngsViz JAR as `tool_path`. Do not guess it from the current analysis working directory.
2. Run a read-only audit:

   ```bash
   python3 /absolute/path/to/installed/ngsviz-setup/scripts/check_ngsviz_environment.py \
...
- If the configured `tool_name` does not exist and the tool root contains exactly one runnable ngsViz JAR, repair the setting with that filename. If no JAR exists, report the error and stop. If several exist and no unique choice is possible, list them and ask the user instead of selecting by filename version or capitalization.
...
- `tool_path/tool_name` is an existing runnable JAR whose `/absolute/path/to/java -jar /absolute/path/to/NGSViz/<configured-tool-name> describe --format json` output declares support for `validate-compute` and `run-compute`.
